...
class SS2D(nn.Module):
    ...
    def forward_corev2(self, x: torch.Tensor, channel_first=False, 
        ...
        # 确保所有参数在正确设备上
        self.x_proj_weight = self.x_proj_weight.to(device)
        self.dt_projs_weight = self.dt_projs_weight.to(device)
        self.dt_projs_bias = self.dt_projs_bias.to(device)
        self.A_logs = self.A_logs.to(device)
        self.Ds = self.Ds.to(device)
        
        if self.ssm_low_rank:
            x = self.in_rank(x)
        
        # 确保传入cross_selective_scan的所有数据都在CUDA上
        x = x.cuda()
        x = cross_selective_scan(
            x, self.x_proj_weight, None, self.dt_projs_weight, self.dt_projs_bias,
            self.A_logs, self.Ds,
    ...
    def forward(self, x: torch.Tensor, **kwargs):
        device = x.device
        # Move input and all module parameters to the same device
        x = x.to(device)
        self.in_proj = self.in_proj.to(device)
        self.act = self.act.to(device)
        if hasattr(self, 'conv2d'):
            self.conv2d = self.conv2d.to(device)
        ...
        if not self.disable_z:
            ...
            if not self.disable_z_act:
                z1 = self.act(z)
                z1 = z1.to(device)  # Ensure z1 is on correct device
        ...
        if not self.disable_z:
            y = y.to(device)  # Ensure y is on same device as z1
            y = y * z1
